...
public class ObjectUtil {
     ...
     * @return
     */
    public static <M> List<String[]> toBeanList(String[] columnProperty , List<Map<String,Object>> mapList) {
    	List<String[]> rsList = new ArrayList<String[]>(mapList.size());
        String[] r = null;
        for (Map<String,Object> map : mapList) {
        	r = new String[columnProperty.length];
        	for(int i = 0 ; i < columnProperty.length ; i++){
        		Object value = map.get(columnProperty[i]);
        		if(value==null) value = "";
        		r[i] = String.valueOf(value);
        	}
        	rsList.add(r);
        }
        return rsList;
